...
**Last revised:** September 22, 2026 (inlet-sdk 0.1.3: CR-092, CR-100 and CR-113 amended, CR-114 to CR-117 added)
...
- **CR-115:** The Electron renderer entry shall detect the application's own code under the `file:` protocol, which is how a packaged application loads. Deriving the root from the document's origin yields a value that matches no frame and marks the whole stack external, which is a failure that appears only in packaged builds.
