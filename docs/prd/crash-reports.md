...
- Any respondent identity beyond an optional integrator-supplied opaque user ID and, under CR-118, the random installation and session IDs of the shared SDK identity (Foundations FD-016).
- Per-user MCP access.
...
- **CR-002:** A crash database shall carry a retention setting: a maximum number of retained reports (default 10,000; platform bounds 1,000 to 100,000) and a maximum report age in days (default 90; bounds 7 to 365, or unlimited). Groups, releases and rollups are not subject to retention. The deployment may widen or narrow the bounds (Foundations FD-032).
...
- **CR-015:** The server shall record the received time and the reporting credential ID. It shall not record the request IP address on a crash report, and the request log of the ingest routes shall carry neither the address nor the port (Foundations §12.2). Every crash route shall be logged by its route pattern rather than its address, so that no installation, session or user ID in a query string reaches the log (UX Analytics AN-019).
- **CR-016:** Ingest shall be rate limited per credential over five-minute and hourly windows, and per credential and fingerprint after the first ten reports of that fingerprint in an hour. Exceeding a limit returns `429` with `Retry-After` and counts toward CR-004. Limits are platform-defined; platform users cannot configure them, and the deployment operator may override them (Foundations FD-032).
...
- **CR-051:** The message headline shall be `kind · exception type or native fault · top in-app frame or faulting module · release`, followed by the group's count and the link to the group. Message text from the envelope is not sent; it may contain content. Neither is any identity field.
...
- **CR-097:** The SDK shall persist queued events across restarts: on disk under a directory the integrator names on Node and Electron, in IndexedDB in browsers, and in the store the integrator injects on React Native. On React Native the fatal path writes synchronously only when that store offers synchronous access, as MMKV does; with an asynchronous store such as AsyncStorage the fatal write is best effort, and the documentation says so. Fatal-path handlers shall write to the persistent queue synchronously before any network call. The queue holds at most 200 events and drops the oldest.
...
- **CR-109:** The bare entry and the entries intended for a browser, an Electron renderer or React Native shall contain no Node imports, direct or transitive, and the Electron renderer installer shall be published from such an entry. The build shall fail if any of them gains one.
...
- **CR-111:** The IPC channel is a trust boundary, because a renderer may run remote content. The main process shall read only the kind, the exception, the context, the tags and the fingerprint from a renderer's payload and shall ignore every other field, so that a renderer cannot forge the release, environment, operating system, runtime, event ID or timestamp and thereby corrupt grouping and regression detection, nor set a user ID through a report. The user ID a report carries is the shared one of Foundations FD-016, which a renderer may set through the analytics renderer entry's `setUserId` unless the main process is installed not to accept it (UX Analytics AN-238), because signing in usually happens in a renderer. It shall accept only the kinds a renderer can legitimately produce, bound tag count and key and value lengths, and accept an optional tag allowlist from the host.
- **CR-112:** The package shall have a root export, so that importing `inlet-sdk` resolves.
- **CR-113:** The SDK shall export its redaction policies by name — the private default, a builder taking the integrator's own safe shapes, one that redacts by pattern (CR-117), and one that sends messages verbatim — so that choosing a redaction level is a deliberate and greppable choice rather than an inline function.
- **CR-114:** The Electron adapter shall not report an exit reason that is not a crash. A renderer or child process that exited with a zero exit code (`clean-exit`) shall not be reported, because that is what closing a window looks like and reporting it makes the highest-volume noise source in the capability. A *killed* child process shall not be reported either, because that is ordinarily the application terminating its own sidecar; a *killed* renderer shall be, because there it is the operating system reclaiming memory. Both lists shall be replaceable by the host, and an empty list shall report every reason.
...
Shared tools already exist for members, invitations, notification settings and the deletion impact; they accept a crash database ID. `get_crash_report` returns the identity fields, and `list_crash_groups` and `list_crash_reports` accept installation and session IDs as filters.
...
- The only identity is the integrator's opaque user ID, and only when the integrator sends it, and, under CR-118, the SDK's random installation and session IDs; none is inferred.
...
- The Electron adapter reports a `render-process-gone` with the reason and exit code, except for the reasons that are not crashes (CR-114), receives a renderer error boundary's envelope through main, and stores its queue under the user-data directory.
...
- *The unclean-exit sentinel is opt-in and packaged-only.* It writes a file and hooks the quit sequence, which is too much to start doing unasked; and a development runner restarts the main process constantly, so arming it there would report the development loop as a crash.
- *An unreadable sentinel still reports.* The previous run died either way. An unknown uptime is a smaller loss than a lost crash.
...
| Renderer process gone | `installElectronMain()` observes `render-process-gone`, ignoring `clean-exit` (CR-114) | `renderer-gone` with `exit.reason`, `exit.code` |
...
| Unclean exit sentinel | `installElectronMain({ uncleanExit: true })`, packaged builds only (CR-116) | `unclean-exit` with `exit.lastUptimeMs` |
