...
## 1. Executive Summary
Inlet is a self-hosted platform that gives a product team one place their applications report to. It began with feedback: forms designed once, collected from any client or from a shared link, read in one interface, exported as data, and operated by an AI agent through MCP. It now also collects crash reports and, from Release 8, product usage events, so that what users say, what breaks and what users do can be read side by side. From Release 9, which ships before Release 8, it also sends something back: a remote configuration that changes what a shipped application does, for everyone or for the users a team chooses, without a release. Every capability shares the same foundations described on this page: projects, typed databases, project-owned API keys, three roles at two scopes, invitation-only accounts, notifications, export, deletion with asynchronous purge, rate limits that platform users cannot configure, one Docker deployment with optional services behind compose profiles, one brand, and one SDK.

A capability is a **database type**. A project holds feedback, crash, analytics and config databases side by side. They share identity, access, credentials, notifications, export, deletion and MCP conventions, and differ only in what they collect or deliver, how it is stored, how it is read, and how long it is kept. This page defines the shared part once. Each capability PRD defines its own part and nothing else.
...
- **Publishable Client Key:** A project-owned credential safe to embed in browser or mobile clients and restricted to the client flows of each database type: the client feedback flow, crash ingest, analytics ingest and the config fetch.
- **Secret Server Key:** A project-owned secret credential that carries project Admin authority over the API and MCP.
- **Database:** A typed collection inside a project. The type is `feedback`, `crash`, `analytics` or `config`. Every database has a stable public ID, a name, database-level memberships, notification settings, an export, a retention rule and a deletion path. The type decides the collection or delivery endpoint, the stored record and where it is stored, the reading interface and the retention default. Analytics events and the records derived from them live in the ClickHouse event store; files live in object storage; everything else lives in PostgreSQL.
...
- **Database ID:** The stable public identifier a client combines with a project credential to target one database. Prefixed by type (`fdb_`, `cdb_`, `adb_`, `cfg_`).
...
- **SDK Identity:** The session ID, user ID and installation ID that `inlet-sdk` shares between its modules (FD-016).
...
- **FR-082:** Publishable client keys may be embedded in public browser or mobile applications. They shall authorize only retrieval of the project's published forms, creation of submission intents, screenshot uploads for those intents, and final submission; the ingest routes of crash and analytics databases (Crash Reports CR-010, UX Analytics AN-010); and the fetch route of config databases, which returns the values a published config resolves to for the context the caller sends, and nothing else (Remote Config RC-040).
...
- **FR-087:** The MVP shall not provide user-configurable scopes, expiration dates, origin restrictions, or usage quotas. A bound a capability sets on what it stores, such as the number of distinct event names an analytics database accepts, is a data bound, not a usage quota.
- **FR-088:** Platform security rate limits shall apply to both credential types, invitation redemption, and public submission-intent creation. Platform users cannot configure them; the deployment operator may override the limits of collection routes and of the config fetch within the bounds of FD-032.
### 8.11 MCP Access
...
- Publishable client keys authorize only the client flows of each database type: collection, and the delivery of a published config.
...
- Database ID, of any type (exactly one of feedback database ID, crash database ID, analytics database ID, config database ID)
- User ID
- Role: Admin, Creator, or Viewer
...
- Publishable client keys may be embedded in browser or mobile clients but authorize only the client flows: collection, and the delivery of a published config.
- Secret server keys carry project Admin authority within their project.
...
- Security rate limits, which platform users cannot configure (FD-032), and abuse protection shall apply to public API operations, especially submission-intent creation, uploads, sign-in, invitation redemption, collection routes and the config fetch.
...
- The platform shall not derive geographic location from IP addresses, with one exception: an analytics database may derive the country, and nothing finer, of an ingest request, and a config database of a fetch, holding the address in memory for that lookup only (UX Analytics AN-033, Remote Config RC-045). No capability stores or logs the address of a crash or analytics ingest request or of a config fetch, nor the context a config fetch carries, and the API logs every route by its route pattern, so that no identifier in a path or a query string reaches the log (UX Analytics AN-019).
...
## 15. Success Criteria
Inlet is successful as a platform when a second capability ships without a new credential type, a new role, a new notification pipeline, a new required deployment service or a new SDK package. The measure is the diff: a capability adds tables, routes, tools and a page, and changes nothing about accounts, keys or roles, nor anything about the deployment beyond configuration values, bundled data such as the IP-to-country database of UX Analytics and Remote Config, and an optional service listed in FD-009 that stays off until the operator enables it, such as the ClickHouse event store of UX Analytics.
...
- Publishable keys are restricted to client flows — the feedback flow when this was decided, crash and analytics ingest and the config fetch since (FR-082); server keys may use all supported project-scoped operations.
- Platform security rate limits that platform users cannot configure are part of the MVP; since Release 8 the deployment operator may override them (FD-032).
...
**Decided September 24, 2026, with Release 8 — UX Analytics**
...
**Decided September 26, 2026, with Release 9 — Remote Config**
...
### 20.2 Values
- **Plain.** Forms are simple, answers are JSON, numbers are exact. Every feature must be explainable in one sentence.
...
- **Density:** compact tables for tabular data. The responses list is deliberately not a table — see section 24 of the Feedback Collection PRD. Submission detail leads with the screenshot, answers beside it, metadata below.
...
## 23. Notifications
> Written as "Slack Notifications" for feedback databases in the unified PRD and shipped in Release 4. It applies unchanged to every database type: the settings row, the write-only webhook, the exact-origin allowlist, queueing inside the storing transaction, retry with backoff, permanent-error stop, escaping of user-authored text, the test message and the visible outcome. What each type announces, and when, is defined by that type: a feedback database announces every new response with a content level; a crash database announces a new group and a regression, never an occurrence; an analytics database announces a data-health incident opening and resolving, never an event; a config database announces a publish, a rollback and an unpublish, never a fetch. FD-006 generalizes the delivery row.
### 23.1 Rationale
...
- **FR-171:** The platform shall never send the observed IP address, the supplied client context, or the installation, session or user ID of the SDK identity to Slack at any content level.
...
- **FD-001:** A project shall hold databases of type `feedback`, `crash`, `analytics` or `config`. The type is fixed at creation and shown wherever the database is named.
- **FD-002:** Every database type shall provide the same shared surface: stable ID, name, rename, database-level memberships and invitations, notification settings, export, deletion impact, deletion with warning and export offer, asynchronous purge of stored objects, rate limits on its client endpoints — its collection endpoints, or the config fetch — that platform users cannot configure (FD-032), and MCP tools for every permitted operation.
...
- **FD-004:** Retention shall be a per-database setting, with a default per type and platform bounds. Feedback defaults to indefinite. Each capability PRD states its default and bounds. A type whose records do not grow with use may keep them for the database's life without a setting, as config versions are kept (Remote Config RC-004).
...
- **FD-012:** The SDK shall have one transport shared by every module: a persistent offline queue (disk on Node and Electron, IndexedDB in browsers, an injected store on React Native), replay on start, exponential backoff on transport failure, a hard stop on `429` that honours `Retry-After` before replay resumes, at least 100 ms between replayed requests, and no retry of an individual event the server has answered. Size limits are enforced before an event is queued. Each module sets its own batch size and queue bound — 50 reports and 200 queued for crashes, 20 pending submissions for feedback, 50 events and 1,000 queued for analytics — and a `429` pauses only the module and route that received it. The config module reads rather than sends: its fetch uses the transport's timeouts, backoff and `429` pause, and has no queue (Remote Config RC-121).
...
- **FD-014:** Nothing the SDK sends automatically may contain content the integrator did not name in the capability's envelope. Each module documents its allowlist and exposes a `beforeSend` hook for redaction, except the config module, whose fetch carries only the context Remote Config section 9.2 names and the attributes the integrator passes. The identity of FD-016 is part of every module's allowlist, under the rules stated there.
- **FD-015:** A route a publishable key may call — a collection route, or the config fetch — may answer cross-origin requests, with a wildcard origin, credentials off and `Retry-After` exposed, because the browser adapters of the SDK run on the integrator's origin and a publishable key was always meant to travel in public code. Nothing else answers cross-origin: not the management interface, not a route a secret key reads, not the hosted form routes. The set of open routes is enumerated in one place in the API and pinned by a test, and widening it is a change to this page first. As of Releases 8 and 9 the set is crash ingest, analytics ingest and the config fetch, the last two matched by method as well as path, the health probe, and the four feedback collection routes (retrieve the published form, create an intent, upload or release an attachment under it, finalize). `/v1/health` names each open capability — `analytics` only once its event store is configured and ready (FD-009) — and lists `identity` when the deployment accepts the fields of FD-016, so an SDK can tell an old deployment from an unreachable one.
- **FD-016:** `inlet-sdk` shall hold one identity per application, shared by every module: a **session ID**, a **user ID** and, once the analytics or config module has created one, an **installation ID**. The installation ID is a random UUID that the analytics module creates in device mode at its first enable, or the config module in device mode at its first fetch unless initialised with `installationId: false` (Remote Config RC-119), whichever comes first, the other adopting it; it is kept until `forget`, and never derives from the device, the network or the user. Both IDs are sent as lowercase dashed UUIDs. The session ID is a random, time-ordered UUID that rotates after a period without activity, 30 minutes by default, after 24 hours, and at each process start outside browsers; activity in any module extends it. While an analytics client is enabled in a browser, the session is shared by every tab of an origin and rotates under a Web Lock; where Web Locks are unavailable the next session ID is derived from the previous one, and is then not time-ordered (UX Analytics AN-229). The user ID is set once for every module, by any module's `setUser` or `setUserId`, and lives in memory. Unless initialised with `identity: false`, the crash and feedback modules attach the session ID, the user ID when one is set, and the installation ID only while an analytics client of the same application is enabled, which they decide by that state and never by an installation ID being present. Identity is written to the device only while an analytics client is enabled: without one, the session ID lives in memory, rotating as above, a new process or page load begins a new one and no two tabs share it; while analytics is disabled the only value it writes is its opt-out choice. The config module is the one exception: it writes the installation ID, and no other identity, so that its rollouts stay stable from one launch to the next, and holds it apart from the installation ID the crash and feedback modules attach (Remote Config RC-119). A config fetch is not activity: it neither starts nor extends a session. It is written where a crash's fatal path can read it synchronously — `localStorage` in browsers, a file on Node and in the Electron main process — and on React Native it is kept in memory and written through to the store injected into the analytics or config module, while crash flags are written to the crash module's store on its fatal path, synchronously only when that store is. It is kept under one key that every module reads, so that each module finds the installation ID another created: modules sharing an identity must be given the same persistence directory on Node and the same store on React Native, which the documentation of each module says first. Electron renderers hold no identity; the main process supplies it. A module sends identity fields only to a deployment whose `/v1/health` lists `identity`, checking when it sends and again after a failed probe, so that a newer SDK never loses a report to an older server's strict envelope. `forget` removes the installation ID, the session ID and pending crash flags, and the installation ID from reports and submissions still queued; a config module creates a new installation ID at its next fetch.
